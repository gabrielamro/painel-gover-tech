...
if (localStorage.getItem('painelpro-dataset-version') !== DATASET_VERSION) {
  ...
  ['painelpro-db', 'painelpro-cadastros', 'painelpro-labels', 'painelpro-label-colors', 'painelpro-system-colors', 'painelpro-relationships', 'painelpro-pf-improvements', 'painelpro-kanban-saved-view', 'painelpro-official-systems-v1', 'painelpro-delivered-data-version'].forEach((key) => localStorage.removeItem(key));
...
const DELIVERED_DATA_VERSION = 'delivered-sprints-2026-09-01-v2';
const deliveredRows = [
  { sprintNumber: 11, order: '15672', project: 'SPR-MCI', detailedPf: 15.75 },
  { sprintNumber: 16, order: '15891', project: 'SPR-MAPI', detailedPf: 12.75 },
  { sprintNumber: 15, order: '15788', project: 'SPR-MAPI', detailedPf: 34.5 },
  { sprintNumber: 12, order: '15787', project: 'SPR-MCI', detailedPf: 20.25 },
  { sprintNumber: 10, order: '15537', project: 'SCIEX Exportação', detailedPf: 49, critical: true },
  { sprintNumber: 13, order: '15646', project: 'SCIEX Portal Único', detailedPf: 57.5, critical: true },
];
if (localStorage.getItem('painelpro-delivered-data-version') !== DELIVERED_DATA_VERSION) {
  const now = new Date().toISOString();
  deliveredRows.forEach((row, index) => {
    const serviceOrder = `#${row.order}`;
    let sprint = sprints.find((item) => String(item.serviceOrder || '').replace(/\D/g, '') === row.order);
    if (!sprint) {
      sprint = {
        code: `OS-${row.order}`,
        objective: `Entrega da Sprint ${row.sprintNumber}`,
        po: 'Não informado',
        end: 'Não informado',
        tasks: 0,
        blocked: 0,
        ...
        taskItems: [],
        technicalLead: 'Não informado',
        nextMilestone: 'Preparar faturamento',
        acceptanceCriteria: 'Entrega registrada e disponível para faturamento.',
        position: sprints.length + index + 1,
      };
      sprints.push(sprint);
    }
    Object.assign(sprint, {
      ...
      project: row.project,
      functionPoints: 0,
      detailedFunctionPoints: row.detailedPf,
      projectManager: "William D'Angelo",
      manager: "William D'Angelo",
      lane: 'billing',
      sourceStatus: 'ENTREGUE',
      deliveryStatus: 'Entregue',
      progress: 100,
      expectedProgress: 100,
      deliveries: Math.max(1, Number(sprint.deliveries || 0)),
      health: row.critical ? 55 : 90,
      priorityLevel: row.critical ? 'Crítica' : (sprint.priorityLevel || 'Média'),
      lastUpdated: now,
      enteredLaneAt: now,
      billingForecastMonth: sprint.billingForecastMonth || PfForecastService.normalizeMonth(''),
    });
  });
  localStorage.setItem('painelpro-sprints', JSON.stringify(sprints));
  localStorage.setItem('painelpro-delivered-data-version', DELIVERED_DATA_VERSION);
}
...
syncDerived();
const canonicalProjectManager = (value) => {
  const key = String(value || '').trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase('pt-BR').replace(/[^a-z]+/g, ' ').trim();
  return ['william', 'willian', 'william d angelo', 'willian d angelo'].includes(key) ? "William D'Angelo" : String(value || '').trim();
};
sprints.forEach((sprint) => {
  if (canonicalProjectManager(sprint.projectManager) === "William D'Angelo" || canonicalProjectManager(sprint.manager) === "William D'Angelo") {
    sprint.projectManager = "William D'Angelo";
    sprint.manager = "William D'Angelo";
  }
});
...
const registry = () => {
  ...
  return {
    ...
    managers: [...new Set([...(saved.managers || []).map(canonicalProjectManager), 'Adilson Villar', "William D'Angelo"])].sort((a, b) => a.localeCompare(b, 'pt-BR')),
...
const relationships = () => { try { const saved = JSON.parse(localStorage.getItem(relationshipKey) || '{}') || {}; return { managers: [...new Set((saved.managers || []).map(canonicalProjectManager))], systemPO: saved.systemPO || {}, poManager: Object.fromEntries(Object.entries(saved.poManager || {}).map(([po, manager]) => [po, canonicalProjectManager(manager)])), systemAnalysts: saved.systemAnalysts || {} }; } catch { return { managers: [], systemPO: {}, poManager: {}, systemAnalysts: {} }; } };
const saveRelationships = (value) => localStorage.setItem(relationshipKey, JSON.stringify(value));
{
  const links = relationships();
  const needsManager = !links.managers.includes('Adilson Villar') || !links.managers.includes("William D'Angelo");
  links.managers = [...new Set([...links.managers, 'Adilson Villar', "William D'Angelo"])];
...
const visible = () => sprints.filter((sprint) => {
  ...
  const matchesManager = ['Gerentes', 'Todos os gerentes'].includes(filters.manager) || (sprint.projectManager || sprint.manager || poManager(sprint.po)) === filters.manager;
...
const pfNumber = (value) => new Intl.NumberFormat('pt-BR', { maximumFractionDigits: 2 }).format(Number(value || 0));
...
function pfFilteredSprints() {
  return sprints.filter((sprint) => {
    ...
    return sprint.billingForecastMonth === pfFilters.month
      ...
      && (pfFilters.manager === 'Gerentes' || (sprint.projectManager || sprint.manager || poManager(sprint.po)) === pfFilters.manager)
...
function bindP0() {
  ...
  if (detailGrid && selected) detailGrid.insertAdjacentHTML('beforeend', `<div><small>Gerente de Projetos</small><b>${esc(selected.projectManager || selected.manager || poManager(selected.po) || 'Não vinculado')}</b></div>`);
